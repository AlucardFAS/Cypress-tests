...
describe('Open Weather', () => {
  ...
  it('Validar status 400 quando latitude for inválida', () => {
    cy.fixture('citys').as('city').then( function () {
      cy.getWeather('a',(this.city.embuGuacu.longitude),false)
        .then((response) => {
  ...
  it('Validar status 400 quando longitude for inválida', () => {
    cy.fixture('citys').as('city').then( function () {
      cy.getWeather(this.city.embuGuacu.latitude,'a',false)
      .then((response) => {
        expect(response.status).to.equal(400);
      })
    })
  ...
  it('Validar status 401 quando API Key inválida', () => {
    cy.fixture('citys').as('city').then( function () {
      cy.getWeather((this.city.embuGuacu.latitude),(this.city.embuGuacu.longitude),false,'a')
      .then((response) => {
